<template>
	<div class="main-content">
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"90%","margin":"0 auto 20px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"margin":"20px 0","display":"flex"}'>
					<el-button :style='{"border":"2px solid rgba(38, 93, 248, 1)","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"rgba(38, 93, 248, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('guidangjianmu','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"2px solid rgba(38, 93, 248, 1)","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"rgba(38, 93, 248, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('guidangjianmu','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"2px solid rgba(38, 93, 248, 1)","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(38, 93, 248, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('guidangjianmu','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"2px solid rgba(38, 93, 248, 1)","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(38, 93, 248, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('guidangjianmu','提交入库')" type="success" size="mini" @click="danganrukuCrossAddOrUpdateHandler(scope.row,'cross','','','')">提交入库</el-button>
							<el-button :style='{"border":"2px solid rgba(38, 93, 248, 1)","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(38, 93, 248, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('guidangjianmu','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"2px solid rgba(38, 93, 248, 1)","cursor":"pointer","padding":"0 24px","margin":"0 10px 5px 0","outline":"none","color":"rgba(38, 93, 248, 1)","borderRadius":"40px","background":"#fff","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('guidangjianmu','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<danganruku-cross-add-or-update v-if="danganrukuCrossAddOrUpdateFlag" :parent="this" ref="danganrukuCrossaddOrUpdate"></danganruku-cross-add-or-update>
...
import danganrukuCrossAddOrUpdate from "../danganruku/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      danganrukuCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    danganrukuCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.danganrukuCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','guidangjianmu');
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.danganrukuCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.danganrukuCrossaddOrUpdate.init(row.id,type);
      });
    },
